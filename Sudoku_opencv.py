...
# Source code:
# https://pyimagesearch.com/2020/08/10/opencv-sudoku-solver-and-ocr/#pyuni-reco-header

def find_puzzle(image, show=False):
...
def extract_digit(cell, show=False):
	...
	if show:
		cv2.imshow("9.Digit", digit)
		#cv2.imshow("Digit", np.concatenate((thresh, digit), axis=1))
		cv2.waitKey(0)

	# return the digit to the calling function
	return digit

def sudoku_ocr(model, image, show_image_proccessing, show_digits_extract):
	image = imutils.resize(image, width=600)

	# find the puzzle in the image and then
	(puzzleImage, warped) = find_puzzle(image, show=show_image_proccessing)
	...
	for y in range(0, 9):
		...
		for x in range(0, 9):
			...
			digit = extract_digit(cell, show=show_digits_extract)
